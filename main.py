...
class RelacoesDAO(object):
    ...
    def create_relation_item_prateleira(self, item, prateleira, info):
        return self.db.execute_query('MATCH (i:item {codigo:$codigo}), (p:prateleira {numero:$numero}) CREATE (i)-[r:GUARDADO_EM {data:$data}]->(p) RETURN i, p, r',
    ...
    def update_relation_item_prateleira(self, item, prateleira, info):
        return self.db.execute_query('MATCH (i:item {codigo:$codigo})-[r]->(p:prateleira {numero:$numero}) SET r.data=$data RETURN r',
                                     {'codigo': item['codigo'], 'numero': prateleira['numero'], 'data': info['data']})
    ...
    def delete_relation_item_prateleira(self, item, prateleira):
        return self.db.execute_query('MATCH (i:item {codigo:$codigo})-[r:GUARDADO_EM]->(p:prateleira {numero:$numero}) DELETE r',
                                     {'codigo': item['codigo'], 'numero': prateleira['numero']})
